defmodule SwiftBetWeb.BetSlip.BetHistoryLive do
  ...
  def render(assigns) do
    ...
    <div class="flex flex-col  justify-center  items-center   ">
      <div class="flex flex-col  justify-end items-start ">
        ...
        <%= if @win_and_lose_chance == 1  do %>
          <h2 class="text-3xl text-green-600  font-semibold mb-4 "> win</h2>
        <% else %>
          <h2 class="text-3xl text-red-600  font-semibold mb-4 ">lose</h2>
        <% end %>
      </div>
        ...
        <%= for bet <- @slip_list do %>
          <div class="flex flex-col md:flex-row divide-solid divide-slate-300 justify-between items-center  w-full p-10 shadow-sm  shadow-indigo-100 rounded-lg bg-gray-200  ">
            ...
            <div>
              ...
              <p class="whitespace-nowrap rounded-full bg-green-100 w-1/2 px-3.5 py-0.5 text-sm text-green-700  ">
                <%= bet.home_out_come %> - <%= bet.away_out_come %>
              </p>
            </div>

            <div class="mb-6 md:mb-0">
              <%= cond do %>
                <% bet.selected == "home" and bet.home_out_come > bet.away_out_come -> %>
                  <p class="material-symbols-outlined  text-green-400">done</p>
                <% bet.selected == "home" and bet.home_out_come < bet.away_out_come -> %>
                  <p class="material-symbols-outlined  text-red-400">close</p>
                  <% bet.selected == "home" and bet.home_out_come == bet.away_out_come -> %>
                  <p class="material-symbols-outlined  text-red-400">close</p>
                <% bet.selected == "away" and bet.home_out_come > bet.away_out_come -> %>
                  <p class="material-symbols-outlined  text-red-400">close</p>
                <% bet.selected == "away" and bet.away_out_come < bet.home_out_come -> %>
                  <p class="material-symbols-outlined  text-green-400">done</p>
                  <% bet.selected == "away" and bet.away_out_come == bet.home_out_come -> %>
                  <p class="material-symbols-outlined  text-green-400">close</p>
                <% bet.selected == "draw" and bet.home_out_come == bet.away_out_come -> %>
                  <p class="material-symbols-outlined  text-green-400">done</p>
                <% bet.selected == "draw" and bet.home_out_come > bet.away_out_come -> %>
                  <p class="material-symbols-outlined  text-red-400">close</p>
                <% bet.selected == "draw" and bet.home_out_come < bet.away_out_come -> %>
                  <p class="material-symbols-outlined  text-red-400">close</p>
                <% true -> %>
                  <td class="your-custom-class-if-no-condition-met">
                    Your content if no condition is met
                  </td>
              <% end %>
            </div>
          </div>
  ...
  def handle_params(%{"id" => id}, _uri, socket) do
    ...
    {:noreply, assign(socket, slip_list: slip_list, bet_ids: bet_ids, status: bet_status, win_and_lose_chance: check_status)}
  end
